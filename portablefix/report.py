...
def _render_html(data: dict) -> str:
    ...
    cards = "\n".join(_render_action_card(a, language) for a in actions)
    ...
    if data["requires_restart"]:
        ...
        restart_section = f"<h2>{t('report_requires_restart')}</h2><ul>{items}</ul>"
    ...
    if comparison:
        ...
        comparison_section = (
            f"<h2>{t('report_since_last_visit')}</h2>"
            ...
            f"{t('report_actions_then_now')}: {comparison['previous_action_count']} &rarr; {comparison['action_count']}</div>"
        )

    return f"""<!DOCTYPE html>
<html lang="{html.escape(language)}"><head><meta charset="utf-8"><title>PortableFix report {html.escape(data['run_id'])}</title>
