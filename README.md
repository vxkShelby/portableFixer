...
- **Tichá odinštalácia a dve fronty:** Uninstaller z Uninstall záznamu
  v registri rozpozná typ inštalátora. MSI (`WindowsInstaller=1` alebo
  `MsiExec` v príkaze) sa odinštaluje cez
  `msiexec.exe /x {GUID} /qn /norestart /l*v <log>` (kód produktu sa
  overí prísnym vzorom GUID, `MsiExec /I` sa nikdy nespustí, log je v
  `Logs/<run-id>_msi/`). Inno Setup (`unins000.exe` alebo hodnoty
  ...
  by počkal, ale nechal by po sebe priečinok aj `Uninstall.exe`).
